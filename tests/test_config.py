...
def test_one_profile_root_determines_every_application_path_without_defaults(
    tmp_path: Path,
) -> None:
    root = tmp_path / "profile"
    paths = ProjectPaths(root)

    resolved = config.ensure_profile(paths)
...
def test_ensure_profile_does_not_replace_an_existing_config(tmp_path: Path) -> None:
